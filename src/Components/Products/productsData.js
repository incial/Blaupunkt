...
export const PRODUCT_TYPE_OPTIONS = ['All', 'Cables', 'Charging Stations', 'DC Charging Station', 'DC Fast Charging', 'Portable Charging']
export const CHARGING_SPEED_OPTIONS = ['All', '3.7kW', '7.4kW', '11kW', '22kW', '43kW', '50kW', '60kW', '120kW', '150kW', '180kW', '250kW', '350kW']
export const CONNECTOR_TYPE_OPTIONS = ['All', 'Type 1', 'Type 2', 'CCS', 'CHAdeMO', 'CCS1', 'CCS2', 'Multi', 'Universal', 'Dual']
export const PHASE_TYPE_OPTIONS = ['All', 'Single-Phase', 'Three-Phase', 'DC', 'Universal', 'Single - Phase', 'Three - Phase']
...
export const PRODUCTS = [
  ...
  ...((chargingCablesData.modelsData?.models || []).map((model, index) => ({
    ...
    type: 'Cables',
    chargingSpeed: chargingCablesData.specificationsData?.specs?.find(s => s.label === 'Rated Current')?.value || '',
    connectorType: model.connectorType || '',
    phaseType: model.phaseType || '',
    price: undefined // Add price if available in data
  })) || []),
  // Charging Stations
  ...((chargingStationsData.modelsData?.sections?.flatMap((section, sectionIndex) => 
    section.categories.map((model, index) => ({
      id: createUniqueId('station', model.modelCode, `${sectionIndex}-${index}`),
      title: chargingStationsData.title || 'Charging Station',
      specifications: `${model.maximumPower || ''} | ${model.connectorType || ''} | ${model.phaseType || ''}`,
      productCode: model.modelCode || `STATION-${sectionIndex}-${index + 1}`,
  ...
  ...((portableEvChargingData.modelsData || []).map((model, index) => ({
    ...
    specifications: `${model.maximumPower || model.chargingPower || ''} | ${model.connectorType || ''} | ${model.phaseType || model.powerPhase || ''}`,
  ...
  ...((dcFastChargingStationData.modelsData?.models || []).map((model, index) => ({
    id: createUniqueId('dc-fast', model.modelCode, index),
    title: dcFastChargingStationData.title || 'DC Fast Charging Station',
    specifications: `${model.maximumPower || ''} | ${model.connectorType || ''} | ${model.outputVoltage || ''}`,
